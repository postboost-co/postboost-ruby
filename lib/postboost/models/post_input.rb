  ...
  class PostInput
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `PostBoost::PostInput` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `PostBoost::PostInput`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'schedule_now')
        self.schedule_now = attributes[:'schedule_now']
      end
    ...
    def list_invalid_properties
      ...
      if @versions.length < 1
        invalid_properties.push('invalid value for "versions", number of items must be greater than or equal to 1.')
      end
    ...
    def valid?
      ...
      return false if @versions.length < 1
      true
    end
    ...
    def versions=(versions)
      ...
      if versions.length < 1
        fail ArgumentError, 'invalid value for "versions", number of items must be greater than or equal to 1.'
      end
    ...
    def hash
      [versions, accounts, tags, date, time, timezone, schedule, schedule_now, queue].hash
    end
