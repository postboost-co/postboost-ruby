  ...
  class ImageGenerationResponse
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `PostBoost::ImageGenerationResponse` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `PostBoost::ImageGenerationResponse`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'prompt_used')
        self.prompt_used = attributes[:'prompt_used']
      else
        self.prompt_used = nil
      end
    ...
    def list_invalid_properties
      ...
      if @prompt_used.nil?
        invalid_properties.push('invalid value for "prompt_used", prompt_used cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @prompt_used.nil?
    ...
    def hash
      [images, prompt_used, revised_prompt, aspect_ratio, quality, credits_used, credits_remaining].hash
    end
