  ...
  class BlogToSocialResponse
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `PostBoost::BlogToSocialResponse` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `PostBoost::BlogToSocialResponse`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'blog_title')
        self.blog_title = attributes[:'blog_title']
      end

      if attributes.key?(:'blog_excerpt')
        self.blog_excerpt = attributes[:'blog_excerpt']
      end

      if attributes.key?(:'captions')
        if (value = attributes[:'captions']).is_a?(Array)
          self.captions = value
        end
      end

      if attributes.key?(:'post_uuid')
        self.post_uuid = attributes[:'post_uuid']
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          blog_title == o.blog_title &&
          blog_excerpt == o.blog_excerpt &&
          captions == o.captions &&
          post_uuid == o.post_uuid &&
          media == o.media &&
    ...
    def hash
      [blog_title, blog_excerpt, captions, post_uuid, media, credits_used, credits_remaining].hash
    end
